...
$sql_work_plan_info = "SELECT mdl_nir_work_plans.id FROM {nir_work_plans}, {nir} WHERE 
                        mdl_nir_work_plans.nir_id = ? AND mdl_nir.id = mdl_nir_work_plans.nir_id AND mdl_nir.teacher_id = ?";

$work_plan_info = $DB->get_record_sql($sql_work_plan_info, array($work_id, $USER->id));
